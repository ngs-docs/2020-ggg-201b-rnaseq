...
3. Run a model that fits the data to find the differentially expressed (DE) genes
...
tximeta incorporates all of that metadata (e.g. transcript locations, transcript and genome source and version, appropriate chromosome lengths, etc) for each transcriptome. This ensures computational reproducibility by attaching critical annotation information to the data object, such that exact quantifications can be reproduced from raw data (all software versions are also attached to the data object).


If you used a different pipeline, you would have to build a "Summarized Experiment" structure yourself, and its a bit tricky to do, so this is another reason it's often good to use established pipelines: someone may have already written helper scripts to automate computationally difficult steps.
...
colnames(explore) <- c("transcript", "sample", "count")
...
ggplot(explore, aes(transcript, count)) + geom_point()
...
ggplot(explore, aes(transcript, count, col=sample)) + geom_point()
...
```{r}
ggplot(explore, aes(transcript, count)) + geom_point() + facet_wrap(~sample)

```


Even though we're still just doing exploration, we're going to put the data into DESeq2 here. That's because it will make a compact data structure that holds all the counts, as well as the metadata. It makes it way easier to do a lot of other exploration once it's in that structure. 

When you analyse your own data, you can run DESeq2 with a very basic model just to get the easy to use data structures for plotting, then re-run it later with the model you are actually interested in. The data structures we're going to use for exploration are the unmodeled data
...
From: http://dx.doi.org/10.1261/rna.053959.115
