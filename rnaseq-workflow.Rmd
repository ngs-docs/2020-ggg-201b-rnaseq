...
2. ✏️ Please fill out our pre-workshop survey if you have not already done so! Link here: https://forms.gle/4jGFeV7eJpV3exhD6
...
1. To make sure that my data looks approximatly correct
...
- Shrink LFCs: The 'magic' of DESeq2 is the way that it 'shares' information across differnt parts of the dataset in a rigorous way to better estimate the variation of the data from different angles so it can create a better result. It uses this information to change the log fold change value- the amount of difference there is between treatment- for each gene based on all of that information about how variation acts in the dataset as a whole.

This outdated tutorial is no longer useful in terms of commands, and the algorithm of DESeq2 has changed a bit since this was written, BUT, [this tutorial has a lot of good description of those algorithm pieces in a bit more detail](https://hbctraining.github.io/DGE_workshop/lessons/04_DGE_DESeq2_analysis.html)


<div class="alert alert-info">
### DESeq2 Quick Facts
- The underlying algorithm is trying to fit your data to a negative binomial distribution, that is, one that doesn't have negatives (you don't get minus 5 reads), that is discrete (has a countable number of variables), that the total of the variables add to 1 (transcripts from gene 1 + gene 2 + gene 3....gene X = 100% of the transcripts you mapped), and that acts like combining together a bunch of Bernoulli trials (the coin flip distribution)
...
Once our data is inside the `tximeta` data object, we want it to transform the information into a "Summarized Experiment". Here we're very creativley calling our new Summarized Experiment object `se`:
...
Before we run DESeq2 it's useful to look at our *unmodeled* data to make some decisions about how we should run the modeling. 
...
p value or log2 Fold Change, which is easy to do in the rendered matrix.
...
### Common Error ⁉️
The independent filtering function does not work the way most people assume! It is maximizing the number of genes that *simultanously* meet two critical thresholds (the alpha and the p-value). If you simply take your results and filter by those two thresholds sequentially, you will *not* get the same result.
...
Procrastigraphing: excessive creation of data visualizations
From: https://speakerdeck.com/raynamharris/rnaseq-a-five-course-meal
...
### Other useful exploration plots
